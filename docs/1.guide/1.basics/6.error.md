...
> [!TIP]
> 建议使用 `new HTTPError({ headers })` 来包含错误响应中的头：
>
> ```js
> throw new HTTPError({
>   status: 400,
...
> ```
>
> 当抛出错误时，通过 `event.res.headers` 设置的任何[已准备的头](/guide/basics/response#preparing-response)都**不会**包含在错误响应中。作为在知道错误之前需要隐式设置头（例如 CORS 头）的最后手段，你可以使用 `event.res.errHeaders`。内置工具（如 `handleCors`）会自动设置两者。
