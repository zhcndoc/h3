...
:read-more{to="/guide/basics/nested-apps" title="嵌套应用"}
...
- `debug`: 在 HTTP 响应中显示调试堆栈跟踪（在生产环境中可能很危险！）。
- `silent`: 启用后，不会显示未处理异常的控制台错误。
- `allowMalformedURL`: 启用后，带有格式错误的百分号编码 URL 路径的请求（例如 `/foo%`、`/%ZZ`）将会以原始、未解码的 pathname 继续通过，而不是被拒绝并返回 `400 Bad Request`（默认行为）。
- `plugins`:（更多信息请参见 [插件](/guide/advanced/plugins)）
...
const app = new H3({
  ...
  onResponse: (response, event) => {
    console.log("响应:", event.url.pathname, response.status);
  },
  onError: (error, event) => {
    console.error(error);
  },
