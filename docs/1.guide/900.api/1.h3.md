...
- `debug`: 在 HTTP 响应中显示调试堆栈跟踪（在生产环境中使用可能会带来危险！）。
- `silent`: 启用后，不会显示未处理异常的控制台错误。
- `allowMalformedURL`: 启用后，包含格式错误的百分号编码 URL 路径（例如 `/foo%`、`/%ZZ`）的请求将使用原始路径名继续处理，而不会被拒绝并返回 `400 Bad Request`（默认行为）。
- `plugins`:（有关更多信息，请参阅 [插件](/guide/advanced/plugins)）
