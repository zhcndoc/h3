...
如果请求体大小超过限制，则抛出 `413` Request Entity Too Large 响应错误。
...
Checks the request cache header (`If-Modified-Since`) and adds cache response headers (`Last-Modified`, `Cache-Control`). Note: `public` cache control is added by default.
...
如果 `x-forwarded-proto` 请求头设置为 "https"，则返回 "https"。如果该请求头包含以逗号分隔的协议列表，则使用第一个条目。你可以通过将 `xForwardedProto` 设置为 `false` 来禁用此行为。
...
如果 `decode` 选项为 `true`，将使用 `decodeURIComponent` 对匹配到的路由参数进行解码。
