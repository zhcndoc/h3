...
一个以 `/` 开头的**内部** `url` 会通过 `event.app.fetch()`（子请求）进行分派，永远不会离开进程。它会继承传入请求经过过滤的请求头（通过 `getProxyRequestHeaders`）和运行时元数据（`ip`、`waitUntil` 等）。它始终会基于应用自身的源解析：开头连续的分隔符（`//host/x`、`/\host/x`，以及类似 `/\thost/x` 这种会被 URL 解析器缩减为一个分隔符的 C0 交错形式）会被合并为单个 `/`，而不会被读取为权限。
...
如果 `target` 以 `/` 开头，则请求会通过 `event.app.fetch()` 由应用路由器在内部处理，而不是发起外部 HTTP 请求。此类目标始终会基于应用自身的源解析：开头连续的分隔符（`//host/x`、`/\host/x`，以及类似 `/\thost/x` 这种会被 URL 解析器缩减为一个分隔符的 C0 交错形式）会被合并为单个 `/`，而不会被读取为权限。
